    ...
    <AnimatePresence>
      {!hide && (
        <motion.div
          ...
          className="fixed inset-0 z-[200] flex items-center justify-center bg-[var(--color-bg)] overflow-hidden"
          initial={{ opacity: 1 }}
        ...
        >
          ...
          <motion.div
            className="absolute inset-y-0 left-0 w-1/2 bg-[var(--color-bg)] z-[2]"
            initial={{ x: 0 }}
            animate={hide ? { x: "-100%" } : { x: 0 }}
            transition={{ duration: 0.9, ease: [0.85, 0, 0.15, 1] }}
          />
          <motion.div
            className="absolute inset-y-0 right-0 w-1/2 bg-[var(--color-bg)] z-[2]"
            initial={{ x: 0 }}
            animate={hide ? { x: "100%" } : { x: 0 }}
            transition={{ duration: 0.9, ease: [0.85, 0, 0.15, 1] }}
          />
          ...
          <div className="relative z-[3] w-full max-w-2xl px-6 text-center">
            ...
            <div
              ...
              className="font-display font-black text-3xl sm:text-5xl md:text-6xl tracking-tight text-[var(--color-text)]"
            >
            ...
            <div className="mt-10 mx-auto max-w-md">
              <div className="flex justify-between text-[10px] font-mono-tech tracking-widest text-[var(--color-accent)] mb-2">
                <span>COMPILING…</span>
                <span data-testid="preloader-progress">
                  {Math.floor(progress)}%
                </span>
              </div>
              <div className="h-[3px] w-full bg-[var(--color-border)] overflow-hidden rounded-full">
                <motion.div
                  className="h-full bg-[var(--color-accent)]"
                  style={{ width: `${progress}%` }}
                />
              </div>
              <div className="mt-3 text-[10px] font-mono-tech text-[var(--color-text-secondary)] tracking-widest">
